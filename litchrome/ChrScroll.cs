...
namespace litchrome
{
    [litsdk.ActivityInfo(Name = "滚动条移动", IsFront = false, IsWinForm = true, RefFile =ChrLoad.RefFile, Index = 48)]
    public class ChrScroll : litcore.iecef.Scroll
    {
        public override void Execute(ActivityContext context)
        {
            ...
            switch (this.ScrollType)
            {
                case litcore.ictype.ScrollType.Top:
                    ChrLoad.Driver.ExecuteScript("window.scrollTo(0,0)");
                    context.WriteLog($"滚动条移至顶端成功");
                    break;
                case litcore.ictype.ScrollType.Botton:
                    ChrLoad.Driver.ExecuteScript("window.scrollTo(0,document.body.scrollHeight)");
                ...
                case litcore.ictype.ScrollType.UpBy:
                    ...
                    if (int.TryParse(num, out inum))
                    {
                        ...
                        ChrLoad.Driver.ExecuteScript($"window.scrollBy(0,{inum})");
                ...
                case litcore.ictype.ScrollType.Xpath:
                    ...
                    string js = litcore.iecef.XPath.CefScrollIntoViewByXPathJs(xpaths, true);
                    ChrLoad.Driver.ExecuteScript(js);
                    break;
            }
