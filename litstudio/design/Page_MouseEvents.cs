...
namespace litstudio
{
    ...
    {
        private litsdk.Port LinkNodeStartPort { get; set; }
        ...
        private void Canvas_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (this.GetNodeFromPoint(e.Location) is Node node)
            {
                string actOld = Newtonsoft.Json.JsonConvert.SerializeObject(node.Activity, Formatting.Indented, new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Auto });
                node.Activity.ShowConfig();
                string actNew= Newtonsoft.Json.JsonConvert.SerializeObject(node.Activity, Formatting.Indented, new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Auto });
                if (actNew != actOld) this.PageUndoChange();
            }
        }
