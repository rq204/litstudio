...
namespace litui
{
    internal partial class OverlayForm : Form
    {
        ...
        /// 当前正处于高亮状态的元素
        /// </summary>
        internal AutomationElement CurrentHighlightElement;
        ...
        public OverlayForm()
        {
            ...
            string filename = AppDomain.CurrentDomain.BaseDirectory + "hand.cur";
            if (!System.IO.File.Exists(filename) || new System.IO.FileInfo(filename).Length != litui.Properties.Resource.cursor.Length)
            {
                System.IO.File.WriteAllBytes(filename, litui.Properties.Resource.cursor);
            }
            this.Cursor = new Cursor(UISelecter.LoadCursorFromFile(filename));
        }

        private void keyStateDispatcherTimer_Tick(object sender, EventArgs e)
        {
            const int MBUTTON = 0x04;
            if (UISelecter.GetAsyncKeyState(MBUTTON) != 0)
            {
                doDelaySelect();
            }
        }
        ...
        private void doSelect()
        {
            lock(this)
            {
                ...
                {
                    var element = CurrentHighlightElement;
                    //element.RelativeClickPos = CurrentHighlightElementRelativeClickPos;

                    //隐藏窗体并截全屏，以便后期自己绘制红色标记矩形
                    this.Hide();
                    Bitmap CaptureScreenshot =UISelecter.CaptureInformativeScreenshot(element);
                    ...
                    System.Threading.Tasks.Task.Run(()=> {
                        this.Invoke(new Action(()=> {
                            UISelecter.OnSelected?.Invoke(element, CaptureScreenshot);
                        }));
                    });
        ...
        private void dispatcherTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    IntPtr hWnd = UISelecter.GetRootWindow(screenPoint);

                    if (hWnd != IntPtr.Zero)
                    {
                        element = UISelecter.UIAAutomation.FromHandle(hWnd);
                    }
                }
                else
                {
                    try
                    {
                        element = UISelecter.UIAAutomation.FromPoint(screenPoint);
                    }
                    catch (Exception)
                    {
                        if (element == null)
                        {
                            IntPtr hWnd = UISelecter.WindowFromPoint(screenPoint);
                    ...
                    //Wechat /Window[3]
                    AutomationElement find = FindChildIn(element);
                    if (find != null) element = find;
        ...
        private AutomationElement FindChildIn(AutomationElement parent)
        {
            if (parent == null) return null;
            AutomationElement[] childs = parent.FindAllChildren();
            if (childs.Length == 0) return null;

            foreach(AutomationElement aee in childs)
            {
                if (parent.BoundingRectangle.Contains(aee.BoundingRectangle))
                {
                    if (aee.BoundingRectangle.Contains(System.Windows.Forms.Cursor.Position))//如果鼠标在子元素当中，则至少选择该子元素
                    {
                        AutomationElement element = FindChildIn(aee);
                        if (element == null) return aee;
                        else return element;
                    }
                }
            }
